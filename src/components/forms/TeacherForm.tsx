...
const TeacherForm = ({type,data}:{type:"create" | "update"; data?:any}) => {
  ...
  return (
    <form onSubmit={onSubmit} className='flex flex-col gap-8'>
        ...
        <span className="text-xs text-gray-400 font-medium">Personal Inofrmation</span>
        <div className="flex flex-wrap gap-4 justify-between">
        <InputField
        label="First Name"
        name="firstName"
        ...
        <input type="file" id="img" {...register("img")} hidden accept="image/*" />
        {errors.img?.message && <p className="text-xs text-red-400">{errors.img?.message.toString()}</p>}
        </div>
        </div>
        
        <button className="bg-blue-400 cursor-pointer hover:opacity-95 text-white p-2 rounded-md">{type === "create" ? "Create" : "Update"}</button>
    </form>
  )
}
